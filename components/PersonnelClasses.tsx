...
const CLASSES = [
  {
    ...
    icon: (
      <svg className="w-5 h-5 text-lime-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <section className="space-y-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {CLASSES.map((cls) => (
          <div key={cls.title} className="p-8 border border-zinc-800/50 hover:border-lime-500/40 bg-zinc-900/10 hover:bg-zinc-900/30 transition-all cursor-pointer group space-y-4">
            <div className="opacity-50 group-hover:opacity-100 transition-opacity">
              {cls.icon}
            </div>
            <h4 className="text-[12px] font-bold tracking-widest text-zinc-100 group-hover:text-lime-400">{cls.title}</h4>
